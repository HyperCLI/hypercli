    ...
    <>
      ...
      <section className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden bg-[#0B0D0E]">
        ...
        >
          ...
          >
            <h3 className="text-5xl sm:text-6xl text-white mb-8 leading-[1.1] tracking-tight max-w-4xl">
              The fastest way to run AI in production
            </h3>
            <p className="text-xl text-[#9BA0A2] max-w-3xl leading-relaxed">
          ...
          <div className="grid md:grid-cols-2 gap-x-8 gap-y-8">
            {items.map((item, index) => (
              ...
              >
                <motion.div 
                  className="w-14 h-14 rounded-xl bg-[#38D39F]/8 flex items-center justify-center flex-shrink-0 group-hover:bg-[#38D39F]/15 transition-all duration-300"
                  whileHover={{ scale: 1.1 }}
                >
                  <item.icon className="w-7 h-7 text-[#38D39F]" />
                </motion.div>
                <div className="flex-1 pt-1">
                  <h4 className="text-2xl text-white mb-2 group-hover:text-[#38D39F] transition-colors duration-300">
                    {item.title}
                  </h4>
                  <p className="text-lg text-[#9BA0A2]">{item.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
