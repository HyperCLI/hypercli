...
export function CodeSnippetCard({
  ...
  return (
    <GlassCard className={cn("code-snippet-card p-1", className)}>
      <div className="code-snippet-card-header flex items-center gap-2 border-b border-border/50 px-4 py-2">
        <div className="flex gap-1.5">
          <div className="h-3 w-3 rounded-full bg-[#ff5f56]" />
          <div className="h-3 w-3 rounded-full bg-[#ffbd2e]" />
          <div className="h-3 w-3 rounded-full bg-[#27c93f]" />
        </div>
        <span className="code-snippet-card-label ml-2 text-xs text-text-muted">{label}</span>
      </div>
      <pre className={cn("code-snippet-card-pre overflow-x-auto border-0 bg-transparent p-4 text-left text-sm leading-relaxed text-text-secondary", preClassName)}>
        <code>{code}</code>
      </pre>
    </GlassCard>
  );
