...
export function ChatMessageBubble({
  ...
  const toggleInlineAudio = useCallback(() => {
    ...
    if (!audio) return;
    if (audio.paused) { void audio.play(); return; }
    audio.pause();
  }, []);
    ...
    >
      ...
      <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} ${bubblesVariant === "v3" && !isUser ? "flex-1 min-w-0" : ""}`}>
        ...
        <div className={`${bubbleClass}${isStreaming && streamingVariant === "v3" ? " relative overflow-hidden" : ""}`}>
          ...
          {message.thinking && isStreaming && (
            <div className={getThinkingBlockClass(themeVariant)}>
              <button
                ...
                className={getThinkingButtonClass(themeVariant)}
              >
                <Brain className="w-3 h-3" />
                {thinkingOpen ? (
                  <><span>Hide reasoning</span><ChevronDown className="w-3 h-3" /></>
                ) : (
                  <><span>Show reasoning</span><ChevronRight className="w-3 h-3" /></>
                )}
              </button>
          ...
          {message.mediaUrls && message.mediaUrls.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-2">
              {message.mediaUrls.map((url, i) => {
                const isImg = /\.(png|jpe?g|gif|webp|svg)(\?.*)?$/i.test(url) || url.startsWith("data:image/");
                if (isImg) {
                  return (
                    <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                      <img src={url} alt="media" className="max-w-[320px] max-h-[320px] rounded-md object-contain" loading="lazy" />
                    </a>
                  );
                }
                return (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline text-xs">
                    {url.split("/").pop() || "media"}
                  </a>
                );
              })}
          ...
          {/* v2 timestamp: inside bubble after content */}
          {renderTimestamp("inside")}
        </div>

        {/* Timestamp outside bubble (off / v1 / v3) */}
        {renderTimestamp("outside")}
      </div>
    </motion.div>
  );
}
...
export function ChatThinkingIndicator({ variant = "off" }: { variant?: ThinkingVariant }) {
  if (variant === "v1") {
    // Alt 1: pulsing brain + framer-motion shimmer progress bar
    return (
      <div className="flex justify-start">
        <div className="bg-surface-low rounded-lg px-4 py-3 flex flex-col gap-2 border-l-2 border-[#38D39F] min-w-[180px]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#38D39F] font-medium">Thinking...</span>
          </div>
          <div className="relative h-0.5 w-full rounded-full bg-[#38D39F]/15 overflow-hidden">
            <motion.div
              className="absolute inset-y-0 rounded-full bg-[#38D39F]/60"
              style={{ width: "40%" }}
              animate={{ left: ["-40%", "110%"] }}
              transition={{ repeat: Infinity, duration: 1.6, ease: "easeInOut" }}
            />
  ...
  if (variant === "v2") {
    // Alt 2: dark pill with framer-motion staggered blinking dots
    return (
      <div className="flex justify-start">
        <div className="bg-[#2f2f2f] rounded-3xl px-5 py-3.5 flex items-center gap-1.5">
          {[0, 1, 2].map((i) => (
            <motion.span
              key={i}
  ...
  if (variant === "v3") {
    // Alt 3: pulsing sparkle + CSS gradient "Working..." (framer can't animate background-clip)
    return (
      <div className="flex justify-start">
        <div className="flex items-center gap-2.5 py-2 px-1">
          ...
          >
            <Sparkles className="w-5 h-5 text-[#38D39F]" />
          </motion.div>
          <span
            className="text-sm font-medium"
            style={{
              background: "linear-gradient(90deg, #38D39F 0%, #4285f4 33%, #f0c56c 66%, #38D39F 100%)",
              backgroundSize: "300% 100%",
              WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent",
              animation: "gradientSlide 2.4s linear infinite",
            }}
          >
            Working...
          </span>
          <style>{`@keyframes gradientSlide { 0% { background-position: 100% 0 } 100% { background-position: -200% 0 } }`}</style>
        </div>
      </div>
    );
  }

  // Default (off): original bouncing dots
  return (
    <div className="flex justify-start">
