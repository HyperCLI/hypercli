...
import { AgentTokenUsage, resolveAgentTokenUsage } from "./AgentTokenUsage";
...
describe("AgentTokenUsage", () => {
  it("keeps normal usage calm and free of reset messaging", () => {
    ...
    expect(screen.queryByText(/near daily allowance|daily allowance used/i)).not.toBeInTheDocument();
  ...
  it("adds awareness and a capacity action near the limit", () => {
    ...
    expect(screen.getByText("Near daily allowance")).toBeInTheDocument();
  ...
  it("caps exhausted usage and communicates the reached state without color alone", () => {
    ...
    expect(progress).toHaveAttribute("aria-valuetext", "30M / 25M. Daily allowance used.");
  ...
  it("distinguishes collapsed warning states without relying on color", () => {
    ...
    expect(screen.getByRole("status")).toHaveTextContent(/today's token usage is near the daily allowance.*usage since 00:00 UTC/i);
    ...
    expect(screen.getByRole("status")).toHaveTextContent(/today's token usage meets the daily allowance.*usage since 00:00 UTC/i);
