          ...
          {caps.map((cap, idx) => {
            ...
            return (
              <motion.div key={idx} initial={{ opacity: 0, scale: 0.88 }} animate={{ opacity: 1, scale: 1 }}
                ...
                whileHover={{ y: -2 }}
                className={`rounded-lg p-2 border ${cap.enabled ? "border-primary/20 bg-primary/5" : "border-border"}`}>
                <div className="flex items-center gap-1.5 mb-0.5">
                  <motion.div animate={cap.enabled ? { scale: [1, 1.1, 1] } : {}} transition={{ repeat: Infinity, duration: 3, delay: idx * 0.4 }}>
                    <CapIcon className={`w-3 h-3 ${cap.enabled ? "text-primary" : "text-text-muted"}`} />
                  </motion.div>
          ...
          {caps.map((cap, idx) => {
            ...
            return (
              <motion.div key={idx} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                ...
                className={`flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium ${cap.enabled ? "bg-primary/10 text-primary" : "bg-surface-high text-text-muted"}`}>
                <CapIcon className="w-3 h-3" />
                {cap.label}
                {cap.enabled && <motion.span className="w-1 h-1 rounded-full bg-primary"
