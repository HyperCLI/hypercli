...
export function ExecQueueModule({ variant, queue: queueProp }: ExecQueueModuleProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 380, damping: 28, delay: 0.3 }}
      className="relative rounded-lg border border-warning/25 bg-warning/5 p-3 space-y-2">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-semibold text-warning uppercase tracking-wider">Pending Approval</span>
        <motion.span className="text-[10px] px-1.5 py-0.5 rounded-full bg-warning/20 text-warning font-bold"
        ...
        <div className="space-y-1.5">
          {queue.map((exec, idx) => (
            <motion.div key={exec.id} initial={{ x: -16, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.15 + idx * 0.08 }}
              className="rounded-md border border-warning/20 bg-background/50 px-2.5 py-2 space-y-1.5">
              <code className="text-[10px] font-mono text-foreground block truncate">{exec.command}</code>
              <div className="flex items-center gap-1.5">
                <motion.button whileTap={{ scale: 0.9 }} className="text-[10px] px-2 py-0.5 rounded bg-primary/15 text-primary hover:bg-primary/25 font-medium">Approve</motion.button>
                <motion.button whileTap={{ scale: 0.9 }} className="text-[10px] px-2 py-0.5 rounded bg-destructive/15 text-destructive hover:bg-destructive/25 font-medium">Deny</motion.button>
        ...
        <div className="space-y-1">
          {queue.map((exec, idx) => (
            <motion.div key={exec.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.06 }}
              ...
              <Terminal className="w-3 h-3 text-warning shrink-0" />
              <code className="text-[10px] font-mono text-foreground flex-1 truncate">{exec.command}</code>
              <motion.button whileTap={{ scale: 0.85 }} className="text-[10px] text-primary">&#x2713;</motion.button>
              <motion.button whileTap={{ scale: 0.85 }} className="text-[10px] text-destructive">&#x2715;</motion.button>
            </motion.div>
          ))}
        </div>
      ) : (
        <div className="flex items-center gap-2 text-[10px]">
          <motion.span className="w-1.5 h-1.5 rounded-full bg-warning"
          ...
          <motion.button whileTap={{ scale: 0.9 }} className="text-warning font-medium ml-auto hover:underline">Review</motion.button>
        </div>
      )}
    </motion.div>
  );
