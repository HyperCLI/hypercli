...
export function MembersSection({ compact = false, agents = [], agentsLoading = false, onRequestProductUse }: MembersSectionProps) {
  ...
  const loadWorkspaceAccess = React.useCallback(async (workspaceId: string) => {
    ...
    try {
      ...
      setAccessState({
        ...
        snapshot: null,
        loading: false,
        error: describeError(cause, "Unable to load Collection access."),
      });
    }
  }, [workspacesClient]);
  ...
  const subjectLabel = (subject: Pick<WorkspaceAccessEntry, "subjectType" | "subjectId" | "displayName" | "displaySlug">): string => {
    ...
    return subject.displaySlug?.trim()
      || (subject.subjectType === "agent" ? "Collection agent" : "Collection member");
  };
  ...
  const createGrant = async () => {
    ...
    try {
      ...
      if (selectedWorkspaceIdRef.current === targetWorkspaceId) {
        setMutationError(describeError(cause, "Unable to add Collection access."));
      }
    } finally {
      ...
      <section className="flex min-h-[356px] flex-col overflow-hidden rounded-lg border border-border bg-surface-low">
        ...
        <div className="flex flex-1 flex-col px-6 py-5 text-left">
          {loadingWorkspaces || loadingAccess ? (
            <div role="status" className="flex flex-1 items-center justify-center gap-2 text-[12px] text-text-muted"><Loader2 className="h-4 w-4 animate-spin" /> Loading Collection access</div>
          ) : visibleError ? (
            ...
            <div role="status" className="flex flex-1 flex-col items-center justify-center text-center"><HardDrive className="h-5 w-5 text-text-muted" /><p className="mt-2 text-[12px] font-medium text-foreground">No Collections yet</p><p className="mt-1 text-[11px] text-text-muted">Create a Collection in Knowledge Hub.</p></div>
            ...
            <div className="flex flex-1 flex-col justify-center">
              ...
              <p className="mt-3 text-center text-[11px] leading-relaxed text-text-muted">The full direct-access list is available to Collection admins.</p>
            </div>
          ) : !hasFullDirectory ? (
            <div role="status" className="flex flex-1 items-center justify-center text-center text-[12px] text-text-muted">Collection access is unavailable.</div>
          ) : directory.length === 0 ? (
  ...
  return (
    <section aria-labelledby="members-title" className="mx-auto w-full max-w-6xl space-y-6 pb-10 text-left">
      <header className="flex items-start justify-between gap-4 border-b border-border pb-6">
        <div className="flex min-w-0 items-start gap-3.5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-xl text-[13px] leading-relaxed text-text-muted">Review access for {selectedWorkspace ? workspaceName(selectedWorkspace) : "the selected Collection"}. Collection admins can manage the full direct-access list.</p>
          </div>
        </div>
        <TooltipHint label="Refresh Collection access" disabled={!selectedWorkspace || loadingWorkspaces || loadingAccess || busy}>
          <button
            type="button"
            onClick={refreshWorkspaceAccess}
            disabled={!selectedWorkspace || loadingWorkspaces || loadingAccess || busy}
            aria-label="Refresh Collection access"
      ...
      {loadingWorkspaces ? (
        <div role="status" className="flex min-h-44 items-center justify-center gap-2 rounded-2xl border border-border text-[12px] text-text-muted"><Loader2 className="h-4 w-4 animate-spin" /> Loading Knowledge Hub Collections</div>
      ) : connectionError && !selectedWorkspace ? null : !selectedWorkspace ? (
        <div role="status" className="rounded-2xl border border-dashed border-border px-5 py-12 text-center"><HardDrive className="mx-auto h-6 w-6 text-text-muted" /><p className="mt-3 text-[13px] font-medium text-foreground">No Collections available</p><p className="mt-1 text-[11px] text-text-muted">Create a Collection in Knowledge Hub to start managing shared access.</p></div>
      ) : (
        <>
          <div className="grid gap-3 lg:grid-cols-3">
            <SummaryCard label="People" value={loadingAccess || !hasFullDirectory ? "-" : String(activePeople)} detail={hasFullDirectory ? "Active direct user access" : "Available to Collection admins"} icon={UsersRound} accent />
            <SummaryCard label="Agents" value={loadingAccess || !hasFullDirectory ? "-" : String(activeAgents)} detail={hasFullDirectory ? "Active direct agent access" : "Available to Collection admins"} icon={Bot} />
            <SummaryCard label="Your role" value={selectedRole ? selectedRole[0]!.toUpperCase() + selectedRole.slice(1) : "Unknown"} detail={canManageAccess ? "Can manage Collection access" : "Your current Collection access"} icon={ShieldCheck} />
          </div>

          {loadingAccess ? (
            <div role="status" className="flex min-h-44 items-center justify-center gap-2 rounded-2xl border border-border text-[12px] text-text-muted"><Loader2 className="h-4 w-4 animate-spin" /> Loading Collection access</div>
          ) : hasCurrentAccessOnly ? (
            <div className="overflow-hidden rounded-2xl border border-border bg-surface-low/20">
              <div className="border-b border-border px-5 py-4"><h2 className="text-[14px] font-semibold text-foreground">Your Collection access</h2><p className="mt-0.5 text-[11px] text-text-muted">The full direct-access list is available to Collection admins.</p></div>
            ...
            <div role="status" className="rounded-2xl border border-dashed border-border px-5 py-12 text-center text-[12px] text-text-muted">Collection access is unavailable.</div>
          ) : (
            <>
              {canManageAccess && accessOpen ? (
                <div id="workspace-access-form" aria-labelledby="workspace-access-form-title" className="grid gap-3 rounded-2xl border border-border bg-surface-low/25 p-4 xl:grid-cols-[130px_minmax(0,1fr)_150px_190px_auto] xl:items-end">
                  <span id="workspace-access-form-title" className="sr-only">Add Collection access</span>
                  <div>
              ...
              {inactiveGrants.length > 0 ? (
                <div className="overflow-hidden rounded-2xl border border-border bg-surface-low/20">
                  ...
                  <div role="table" aria-label="Inactive Collection access grants">
      ...
      <ConfirmDialog
        ...
        message={pendingCurrentRevoke ? `Remove all direct access for ${subjectLabel(pendingCurrentRevoke)} from ${selectedWorkspace ? workspaceName(selectedWorkspace) : "this Collection"}?` : ""}
