...
**Current:** Frontend computes uptime from `started_at`. Deployment lifecycle is event-driven through the token-returned `ws_url` (currently the canonical `/ws/deployments` endpoint); those invalidations intentionally carry no CPU or memory snapshot. The SDK exposes `Deployments.metrics(id)` for an on-demand backend read, but the dashboard has no metrics stream.
