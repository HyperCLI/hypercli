...
Use `npm run dev:real` to start Claw with environment generated from the shared
local setup script. Local development should exercise the real Backend/SDK
contract rather than an in-repo mock Agents API.
...
**Agent lifecycle**: STOPPED → PENDING → STARTING → RUNNING → STOPPING → STOPPED. The page subscribes through `Deployments.subscribe()` to the token-returned `ws_url` (currently the canonical `/ws/deployments` endpoint), treats flat transition events as invalidations, coalesces bursts, and reloads the authoritative REST list. It performs no deployment-state polling interval.
