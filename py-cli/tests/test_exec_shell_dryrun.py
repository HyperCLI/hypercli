...
def test_agents_start_reuses_saved_launch_fields_but_inherits_backend_sync_policy(monkeypatch):
    ...
    saved_state = {
        agent_id: {
            ...
            "launch_config": {
                ...
                "image": "registry.example.com/hypercli/hypercli-openclaw:untested",
    ...
    assert "--dry-run cannot validate launch overrides on start" in result.output
    assert "updated_agent_id" not in captured
    assert "agent_id" not in captured


def test_agents_start_without_overrides_uses_bodyless_start(monkeypatch):
    calls: list[tuple[str, object]] = []

    def start(agent_id, *, dry_run=False):
    ...
    saved_state = {
        canonical_id: {
            ...
            "launch_config": {
                ...
                "image": "registry.example.com/hypercli/hypercli-openclaw:saved",
            },
        }
    }
