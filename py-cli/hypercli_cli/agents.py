...
def start(
    ...
    if requested_agent_id == "self":
        ...
        try:
            ...
            pod = agents.start(current.id, dry_run=dry_run)
        except Exception as e:
    ...
    if dry_run:
        ...
        if launch_overrides:
            console.print(
                "[red]❌ --dry-run cannot validate launch overrides on start; "
                "update launch_config explicitly first: "
                f"{', '.join(launch_overrides)}[/red]"
            )
            raise typer.Exit(1)
