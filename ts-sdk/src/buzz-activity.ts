 ...
 *
 * Buzz-backed HyperCLI deployments run `hyper-acp plugin buzz`, which publishes observer
  ...
  if (!relayUrl) {
    throw new Error(
      'Agent is not Buzz-backed: launchConfig env has no BUZZ_RELAY_URL; ' +
        'observer activity is only published by Buzz deployments running hyper-acp plugin buzz',
    );
  }
