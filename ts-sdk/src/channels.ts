 ...
 * for `HYPER_SLACK_API_URL`. A value that parses false (`0`) is the explicit
 * disable: the container boot removes the hosted Slack channel config, so the
 * SDK writes `0` for an asserted off rather than relying on the key being
 * absent. `HYPER_AGENTS_API_KEY` is also required at boot but is injected by
 * the platform and stripped from caller-supplied env, so it is deliberately
 ...
 *
 * Enabled is all-or-nothing on purpose: a launch env with a truthy
 * `HYPER_SLACK_APP_ENABLED` and a missing companion kills the pod at boot.
 * Disable is explicit, not absence: `HYPER_SLACK_APP_ENABLED=0` parses false
 * and the container boot removes the hosted Slack channel config, while an
 * unset key means "untouched" rather than "off".
 */
