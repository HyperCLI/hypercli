...
describe('Agent file source switch', () => {
  ...
  it('routes gateway writes to agents.files.set with the resolved gateway agent id', async () => {
    const { agent, gatewayClient, deployments } = makeAgent();
    const res = await agent.fileWrite('AGENTS.md', 'hi there', 'gateway');
    // Uses the in-gateway agent id "main", NOT the deployment id "agent-123".
    expect(gatewayClient.fileSet).toHaveBeenCalledWith('main', 'AGENTS.md', 'hi there');
  ...
  it('routes gateway reads to agents.files.get with the resolved gateway agent id', async () => {
