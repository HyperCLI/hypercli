...
describe('Agents SDK', () => {
  ...
  it('starts without accepting launchConfig compatibility options', async () => {
    ...
    await expect(deployments.start('agent-123', { launchConfig: {} } as any))
      .rejects.toThrow(/start no longer accepts launch mutation options: launchConfig/);
  ...
  it('serves declarative and named route updates for an owned agent id', async () => {
    ...
    await deployments.setRoute(agentId, 'web app', {
      port: 3000,
      auth: false,
      prefix: '',
      remove_headers: ['X-Test'],
    });
    await deployments.setRoute(agentId, 'empty headers', {
      port: 3001,
      auth: false,
      remove_headers: [],
    });
    ...
    expect(http.put).toHaveBeenCalledWith(`/deployments/${agentId}/routes/empty%20headers`, {
      port: 3001,
      auth: false,
    });
  ...
  it('buildAgentConfig omits empty remove_headers from generic routes', () => {
    expect(buildAgentConfig({}, {
      routes: {
        web: { port: 3000, auth: true, prefix: '', remove_headers: [] },
        api: { port: 8000, remove_headers: ['X-Test'] },
      },
    }).config.routes).toEqual({
      web: { port: 3000, auth: true, prefix: '' },
      api: { port: 8000, remove_headers: ['X-Test'] },
    });
  });

  it('buildOpenClawTrustedProxiesEnv writes a comma-separated replacement env', () => {
    expect(buildOpenClawTrustedProxiesEnv(null)).toEqual({});
    expect(buildOpenClawTrustedProxiesEnv([])).toEqual({});
    expect(buildOpenClawTrustedProxiesEnv([' 10.0.0.0/8 ', '', '127.0.0.1']))
      .toEqual({ OPENCLAW_TRUSTED_PROXIES: '10.0.0.0/8,127.0.0.1' });
  });
  ...
  it('rehydrates a redacted projection for explicit update round-trips', async () => {
    ...
    const { deployments } = installStoredProjection(stored, { API_TOKEN: 'tok' });

    const launchConfig = await deployments.storedLaunchConfig(STORED_AGENT_ID);

    expect(launchConfig.secrets).toEqual({ API_TOKEN: 'tok' });
    expect(launchConfig.registry_auth).toEqual({});
    expect(launchConfig.env).toEqual({ MODE: 'prod' });
  ...
  it('honours explicitly empty redactable keys instead of reading secrets back', async () => {
    ...
    expect(calls).toEqual([
      `/deployments/${STORED_AGENT_ID}`,
      `/deployments/${STORED_AGENT_ID}/secrets`,
      `/deployments/${STORED_AGENT_ID}/secrets/API_TOKEN`,
    ]);
    expect(launchConfig.secrets).toEqual({ API_TOKEN: 'tok' });
  });
  ...
  it('leaves a config missing more than the redacted keys to the completeness check', async () => {
    ...
    await expect(deployments.storedLaunchConfig(STORED_AGENT_ID))
      .rejects.toThrow('launchConfig is incomplete; missing: image');
    expect(calls).toEqual([
      `/deployments/${STORED_AGENT_ID}`,
      `/deployments/${STORED_AGENT_ID}/secrets`,
      `/deployments/${STORED_AGENT_ID}/secrets/API_TOKEN`,
  ...
  it('refuses to invent registry_auth for a private registry and accepts a supplied one', async () => {
    const stored: Record<string, any> = buildAgentConfig().config;
    stored.registry_url = 'registry.example.com';
    delete stored.registry_auth;
    const { deployments } = installStoredProjection(stored);
  ...
  it('normalizes a legacy nullable restart when rebuilding the stored launch config', async () => {
    ...
    expect(launchConfig.restart).toBe(false);
    expect(post).not.toHaveBeenCalled();
  });
  ...
  it('resolves unique agent names before lifecycle calls', async () => {
    ...
    expect((http as any).patch).not.toHaveBeenCalled();
  ...
  it('starts OpenClaw Pro without launch mutation', async () => {
    const post = vi.fn().mockResolvedValue({
      id: 'agent-pro',
      user_id: 'user-456',
      state: 'STARTING',
  ...
  it('generic update uses the sync root without an independent full-sync knob', async () => {
    const post = vi.fn().mockResolvedValue({
      id: 'agent-sync-all',
      user_id: 'user-456',
      state: 'STARTING',
  ...
  it('distinguishes omitted and explicit null sync fields when updating', async () => {
    const post = vi.fn().mockResolvedValue({
      id: 'agent-sync-presence',
      user_id: 'user-456',
      state: 'STARTING',
    ...
    for (const launchConfig of configs) await deployments.update(agentId, { launchConfig });
  ...
  it('retains the backend-hydrated launch config after start', async () => {
    ...
    const result = await deployments.start('11111111-1111-4111-8111-111111111111');
