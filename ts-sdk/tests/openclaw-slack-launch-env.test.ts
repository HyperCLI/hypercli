...
describe('hosted Slack launch env', () => {
  ...
  it('createOpenClaw writes the explicit Slack disable flag for slack: false', async () => {
    const post = vi.fn().mockResolvedValue(createdAgentPayload({ state: 'STOPPED' }));
    const patch = vi.fn();
    const deployments = deploymentsWith({ post, patch });
    ...
    await deployments.createOpenClaw({
      ...
      dryRun: true,
      slack: false,
      // A caller-provided enable flag is overwritten: the SDK owns the set.
      env: { HYPER_SLACK_APP_ENABLED: '1' },
    });

    const postedEnv: Record<string, string> = post.mock.calls[0]?.[1]?.env ?? {};
    expect(postedEnv.HYPER_SLACK_APP_ENABLED).toBe('0');
    expect(Object.keys(postedEnv).filter((key) => key.startsWith('HYPER_SLACK_'))).toEqual([
      'HYPER_SLACK_APP_ENABLED',
    ]);
    // An explicit disable never needs the post-create launch-config patch.
    expect(patch).not.toHaveBeenCalled();
  });

  it('createOpenClaw refuses enable-asserting Slack env alongside slack: false', async () => {
    const post = vi.fn();
    const deployments = deploymentsWith({ post });
    ...
    await expect(deployments.createOpenClaw({
      ...
      /slack conflicts with HYPER_SLACK_RELAY_URL, HYPER_SLACK_API_URL, HYPER_SLACK_GATEWAY_ID in env/,
    );
    expect(post).not.toHaveBeenCalled();
  });

  it('disableOpenClawHostedSlack writes the explicit disable env', async () => {
    const get = vi.fn().mockResolvedValue(createdAgentPayload({
      state: 'STOPPED',
      runtime: 'openclaw',
    }));
    const patch = vi.fn().mockResolvedValue({
      agent_id: AGENT_ID,
      key: 'HYPER_SLACK_APP_ENABLED',
      present: true,
      launch_epoch: 2,
    });
    const deployments = deploymentsWith({ get, patch });

    await deployments.disableOpenClawHostedSlack(AGENT_ID);

    expect(patch).toHaveBeenCalledWith(
      `/deployments/${AGENT_ID}/env/HYPER_SLACK_APP_ENABLED`,
      { value: '0' },
    );
  });

  it('disableOpenClawHostedSlack rejects a non-OpenClaw Agent', async () => {
    const get = vi.fn().mockResolvedValue(createdAgentPayload({ state: 'STOPPED' }));
    const patch = vi.fn();
    const deployments = deploymentsWith({ get, patch });

    await expect(deployments.disableOpenClawHostedSlack(AGENT_ID))
      .rejects.toThrow(`Agent ${AGENT_ID} is not an OpenClaw deployment`);
    expect(patch).not.toHaveBeenCalled();
  });
