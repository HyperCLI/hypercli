...
export const usage = [
  ...
  'hyper jobs create --image IMG [--runtime SEC] [--gpu T] [--region R] [--count N] [--name N] [--env K=V ...] [--constraint K=V ...] [--cpu-vendor V] [--dry-run] -- CMD...',
...
function parseConstraints(entries: unknown, cpuVendor: unknown): Record<string, string> | undefined {
  const constraints: Record<string, string> = {};
  if (Array.isArray(entries)) {
    for (const entry of entries) {
      const eq = String(entry).indexOf('=');
      const key = eq > 0 ? String(entry).slice(0, eq).trim() : '';
      const value = eq > 0 ? String(entry).slice(eq + 1).trim() : '';
      if (!key || !value) throw new UsageError(`invalid --constraint '${String(entry)}' (expected KEY=VALUE)`);
      constraints[key] = value;
    }
  }
  const vendor = str(cpuVendor);
  if (vendor && constraints.cpu_vendor === undefined) constraints.cpu_vendor = vendor;
...
async function create(ctx: CommandContext, args: string[]): Promise<void> {
  ...
  const constraints = parseConstraints(parsed.values.constraint, parsed.values['cpu-vendor']);
  if (constraints) options.constraints = constraints;
