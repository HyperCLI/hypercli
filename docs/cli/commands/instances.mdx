...
Show real-time idle and launching counts by GPU type and region:

```bash
hyper instances capacity
hyper instances capacity --gpu h100 --output json
```

Capacity is a point-in-time observation and does not reserve an instance.
...
| `--port` | `-p` | Raw TCP port(s) to expose (repeatable) |
| `--lb` | | Enable HTTPS load balancer on the given container port |
| `--lb-auth` | | Require Bearer token auth on the HTTPS load balancer |
| `--dockerfile` | `-d` | Build this Dockerfile on the GPU node (remote build) |
| `--follow` | `-f` | Follow logs after launch |
| `--dry-run` | | Validate config/pricing without creating a job or reserving funds |
...
Job tags are supported by the API and SDK through the optional `tags` object, and tagged jobs are returned with `tags` in job responses. The CLI currently supports tag filtering with `hyper jobs list --tag KEY=VALUE`; creation-time tag assignment is not yet exposed on `hyper instances launch`.

### Port Mappings

`--lb` and `--port` are different surfaces:

- `--lb <container_port>`: creates a public **HTTPS** endpoint at `https://<job.hostname>`.
- TLS certificate is terminated by Traefik automatically for that hostname.
- Use this for APIs/webhooks/inference endpoints.

- `-p/--port <port>`: exposes **raw TCP** port(s).
- Use this for non-HTTP protocols and direct TCP access patterns.
- No automatic HTTPS/TLS termination is added by `--port`.

```bash
# HTTPS endpoint via Traefik + TLS cert on job hostname
hyper instances launch my-image:latest --lb 8080

# Raw TCP exposure (repeat as needed)
hyper instances launch my-image:latest -p 2222 -p 9090

# Combine both HTTPS and raw TCP on one job
hyper instances launch my-image:latest --lb 8000 -p 2222
```
...
Validate your configuration, GPU availability, and account balance without launching:

```bash
hyper instances launch my-image:latest -g h100 -n 8 --dry-run
# ✓ Dry run passed
#   GPU:      H100 x8
#   Region:   se
#   Price:    $13.42/hr
#   Cold boot: yes (~15 min)
```

The response includes a `cold_boot` indicator — when `false`, a warm instance is available and your job will start in under a minute.
