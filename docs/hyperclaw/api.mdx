...
```bash
curl -X POST https://api.hypercli.com/api/stripe/1aiu \
  -H "Authorization: Bearer <jwt>" \
  -H "Content-Type: application/json" \
  -d '{
    "success_url": "https://hypercli.com/dashboard/plans?session_id={CHECKOUT_SESSION_ID}",
    "cancel_url": "https://hypercli.com/dashboard/plans?cancelled=true"
  }'
```
...
```bash
# Step 1
curl -X POST https://api.hypercli.com/api/x402/1aiu
...
```bash
curl -X POST https://api.hypercli.com/api/x402/job \
  -H "Content-Type: application/json" \
  -H "X-PAYMENT: <signed-payment-payload>" \
  -d '{
    "gpu_type": "l40s",
...
When tags are set, they are returned on the nested `job` object in the response. HyperClaw's current public spec exposes x402 job creation, not a separate job listing endpoint.
