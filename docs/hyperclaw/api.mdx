...
Live spec: `https://api.agents.hypercli.com/api/openapi.json`
...
```bash
curl https://api.agents.hypercli.com/api/plans
```
...
```bash
curl -X POST https://api.agents.hypercli.com/api/stripe/1aiu \
  -H "Authorization: Bearer <jwt>" \
...
```bash
# Step 1
curl -X POST https://api.agents.hypercli.com/api/x402/1aiu
...
```bash
curl -X POST https://api.agents.hypercli.com/api/x402/job \
  -H "Content-Type: application/json" \
  -H "X-PAYMENT: <signed-payment-payload>" \
  -d '{
    "gpu_type": "l40s",
...
When tags are set, they are returned on the nested `job` object in the response. The canonical published API reference for HyperClaw is the live OpenAPI spec at `https://api.agents.hypercli.com/api/openapi.json`.
