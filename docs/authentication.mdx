...
- selector tags such as `team=dev`, `agent=<agent-uuid>`, `model=kimi-k2.5`, or `voice=alloy`
...
Voice compatibility routes work the same way:

- `POST /v1/tts`
- `POST /v1/clone`
- `POST /v1/design`

Use `voice:*` for broad access, or narrower tags such as `voice=alloy` when you want to pin a key to a specific voice.
