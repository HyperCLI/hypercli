...
job = client.jobs.create(
    ...
    tags={"team": "ml", "env": "prod"},  # optional
)
...
| `tags` | dict[str, str] | None | Optional string tags for organizing and filtering jobs |
...
The SDK sends tag filters as repeated `tag=key:value` query params to `GET /api/jobs`, and job responses include `tags` when present.
...
jobs = client.jobs.list(state="running", tags={"team": "ml", "env": "prod"})
...
| `tags` | dict[str, str] | Optional string tags returned when set on the job |
