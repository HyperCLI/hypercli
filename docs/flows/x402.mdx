...
- If the wallet has no account yet, HyperCLI creates or reuses a wallet-backed user for that wallet and links it into the normal backend identity path.
...
If you want backend-backed history after using a wallet, you have two paths:
...
Or use the wallet context directly on read commands:

```bash
hyper flow list --x402
hyper flow get <render_id> --x402
hyper flow status <render_id> --x402
```
