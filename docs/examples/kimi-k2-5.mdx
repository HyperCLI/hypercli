...
The model supports reasoning (thinking) mode — reasoning content appears in `message.reasoning_content`.

<Note>
For the hosted HyperClaw chat API, prefer the Anthropic Messages endpoint at `https://api.hyperclaw.app` when you care about tool calling quality. The OpenAI-compatible `/v1` route still works, but Anthropic-format routing is the recommended default.
</Note>
