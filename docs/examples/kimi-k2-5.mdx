...
- 8 GPUs required — 4x H200 is not enough (only ~3GB free per GPU after weights)
...
| `--trust-remote-code` | — | Required for Kimi K2.5's custom model code |
| `--tool-call-parser kimi_k2` | — | Native tool/function calling support |
| `--reasoning-parser kimi_k2` | — | Extracts reasoning/thinking content from output |
| `--enable-auto-tool-choice` | — | Automatically detect when to use tools |
| `--mm-encoder-tp-mode data` | — | Data-parallel vision encoder (better perf than TP for small encoders) |
| `--enable-prefix-caching` | — | Cache common prompt prefixes across requests |
| `--enable-chunked-prefill` | — | Overlap prefill with decode for better throughput |
...
For Kimi K2.5, give the model enough output budget to think and still return visible text. A small `max_tokens` cap often gets consumed entirely by reasoning. Reasoning content appears in `message.reasoning_content`.

<Note>
For the hosted HyperClaw chat API, prefer the Anthropic Messages endpoint at `https://api.hypercli.com` when you care about tool calling quality. The OpenAI-compatible `/v1` route still works, but Anthropic-format routing is the recommended default.
</Note>
