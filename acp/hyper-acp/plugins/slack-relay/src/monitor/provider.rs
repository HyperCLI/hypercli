...
fn json_env(name: &str) -> Option<Vec<String>> {
    let raw = env_var_with_legacy(name)?;
...
pub(crate) fn build_policy_from_env(
    ...
    ActiveSlackRelayPolicy {
        account_id,
        current_bot_user_id: env_var_with_legacy(HYPER_ACP_SLACK_BOT_USER_ID_ENV),
        current_bot_id: env_var_with_legacy(HYPER_ACP_SLACK_BOT_ID_ENV),
        group_policy: parse_group_policy_with_default(
            env_var_with_legacy(HYPER_ACP_SLACK_GROUP_POLICY_ENV).as_deref(),
            GroupPolicy::Allowlist,
        ),
        dm_policy: parse_dm_policy_with_default(
            env_var_with_legacy(HYPER_ACP_SLACK_DM_POLICY_ENV).as_deref(),
            DmPolicy::Pairing,
        ),
        ...
        allow_bots: parse_allow_bots(
            env_var_with_legacy(HYPER_ACP_SLACK_ALLOW_BOTS_ENV).as_deref(),
        ),
        reply_to_mode: parse_reply_mode(
            env_var_with_legacy(HYPER_ACP_SLACK_REPLY_TO_MODE_ENV).as_deref(),
        ),
        direct_reply_to_mode: env_var_with_legacy(HYPER_ACP_SLACK_DIRECT_REPLY_TO_MODE_ENV)
            .as_deref()
        ...
        context_visibility: parse_context_visibility(
            env_var_with_legacy(HYPER_ACP_SLACK_CONTEXT_VISIBILITY_ENV).as_deref(),
        ),
        thread_initial_history_limit: env_var_with_legacy(HYPER_ACP_SLACK_THREAD_HISTORY_LIMIT_ENV)
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(20),
        media_max_bytes: env_var_with_legacy(HYPER_ACP_SLACK_MEDIA_MAX_BYTES_ENV)
            .and_then(|value| value.parse::<u64>().ok())
            .unwrap_or(DEFAULT_SLACK_MEDIA_MAX_BYTES),
        direct_client_config: SlackDirectClientConfig::from_env().ok(),
        reaction_mode: env_var_with_legacy("HYPER_ACP_SLACK_REACTION_MODE")
            .unwrap_or_else(|| "all".to_owned()),
        reaction_allowlist_lower: csv_env("HYPER_ACP_SLACK_REACTION_ALLOWLIST")
            .into_iter()
            .map(|value| value.to_ascii_lowercase())
            .collect(),
        expected_team_id: env_var_with_legacy(HYPER_ACP_SLACK_TEAM_ID_ENV),
        expected_api_app_id: env_var_with_legacy(HYPER_ACP_SLACK_API_APP_ID_ENV),
    }
}
...
mod tests {
    ...
    fn legacy_env_name(name: &str) -> String {
        let legacy = name.strip_prefix("HYPER_ACP_SLACK_").unwrap();
        format!("HYPER_SLACK_{legacy}")
    }

    fn clear_env_pair(name: &str) {
        std::env::remove_var(name);
        std::env::remove_var(legacy_env_name(name));
    }

    fn clear_policy_env_test_vars() {
        for name in [
            HYPER_ACP_SLACK_ALLOW_FROM_ENV,
            HYPER_ACP_SLACK_GROUP_POLICY_ENV,
            HYPER_ACP_SLACK_CHANNELS_ENV,
    ...
    #[test]
    fn hosted_policy_env_fallbacks_are_honored_when_acp_absent() {
        let _env = crate::test_env_lock();
        clear_policy_env_test_vars();
        std::env::set_var(
            legacy_env_name(HYPER_ACP_SLACK_ALLOW_FROM_ENV),
            " U1, Alice ",
        );
        std::env::set_var(
            legacy_env_name(HYPER_ACP_SLACK_GROUP_POLICY_ENV),
            "disabled",
        );
        std::env::set_var(
            legacy_env_name(HYPER_ACP_SLACK_CHANNELS_JSON_ENV),
            r#"["Cjson"]"#,
        );
        ...
        clear_policy_env_test_vars();
        assert_eq!(policy.allow_from_lower, vec!["u1", "alice"]);
        assert_eq!(policy.group_policy, GroupPolicy::Disabled);
        assert_eq!(policy.allowed_channel_ids, vec!["Cjson"]);
    }

    #[test]
    fn hosted_channels_csv_fallback_is_honored_when_acp_absent() {
        let _env = crate::test_env_lock();
        clear_policy_env_test_vars();
        std::env::set_var(legacy_env_name(HYPER_ACP_SLACK_CHANNELS_ENV), " C1, C2 ");
    ...
    #[test]
    fn acp_policy_env_wins_over_hosted_fallbacks() {
        let _env = crate::test_env_lock();
        clear_policy_env_test_vars();
        std::env::set_var(HYPER_ACP_SLACK_ALLOW_FROM_ENV, "AcpUser");
        std::env::set_var(
            legacy_env_name(HYPER_ACP_SLACK_ALLOW_FROM_ENV),
            "LegacyUser",
        );
        std::env::set_var(HYPER_ACP_SLACK_GROUP_POLICY_ENV, "open");
        std::env::set_var(
            legacy_env_name(HYPER_ACP_SLACK_GROUP_POLICY_ENV),
            "disabled",
        );
        std::env::set_var(HYPER_ACP_SLACK_CHANNELS_JSON_ENV, r#"["Cacp"]"#);
        std::env::set_var(
            legacy_env_name(HYPER_ACP_SLACK_CHANNELS_JSON_ENV),
            r#"["Clegacy"]"#,
        );
        ...
        clear_policy_env_test_vars();
        assert_eq!(policy.allow_from_lower, vec!["acpuser"]);
        assert_eq!(policy.group_policy, GroupPolicy::Open);
        assert_eq!(policy.allowed_channel_ids, vec!["Cacp"]);
    }
