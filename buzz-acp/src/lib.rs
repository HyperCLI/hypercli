...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("buzz_acp=info,hypercli_buzz_acp=info")),
        )
        .compact()
        .init();
                    ...
                    match buzz_event {
                        Some(buzz_event) => {
                            ...
                            let matched = filter::match_event(
                                ...
                                &rules,
                                &pubkey_hex,
                                config.text_mention_name.as_deref(),
                                is_dm,
                            )
                            .await;
...
fn supports_protocol_auth(method: &serde_json::Value) -> bool {
    matches!(
        method.get("type").and_then(serde_json::Value::as_str),
        None | Some("agent")
    )
}
...
mod auth_method_tests {
    use super::supports_protocol_auth;

    #[test]
    fn protocol_auth_accepts_default_and_explicit_agent_methods() {
        assert!(supports_protocol_auth(
            &serde_json::json!({"id": "default-login"})
        ));
        assert!(supports_protocol_auth(
            &serde_json::json!({"id": "agent-login", "type": "agent"})
        ));
    }

    #[test]
    fn protocol_auth_rejects_terminal_and_unknown_methods() {
        assert!(!supports_protocol_auth(
            &serde_json::json!({"id": "terminal-login", "type": "terminal"})
        ));
        assert!(!supports_protocol_auth(
            &serde_json::json!({"id": "future-login", "type": "future"})
        ));
    }
}
...
async fn run_authenticate(args: AuthenticateArgs) -> Result<()> {
    ...
    let Some(method) = methods.iter().find(|method| {
        method.get("id").and_then(|id| id.as_str()) == Some(args.method_id.as_str())
    }) else {
        client.shutdown().await;
        eprintln!(
    ...
    if !supports_protocol_auth(method) {
        ...
        client.shutdown().await;
        eprintln!(
            "error: auth method '{}' has type '{}'; run its advertised login command instead of ACP authenticate",
            args.method_id, method_type
        );
        std::process::exit(1);
    }
