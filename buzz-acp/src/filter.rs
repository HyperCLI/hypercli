...
pub struct SubscriptionRule {
    ...
    #[serde(default)]
    pub kinds: Vec<u32>,
    /// If `true`, the event must contain an accepted structured or textual mention.
    #[serde(default)]
...
/// 3. **require_mention** — if `true`, an exact structured mention or an enabled,
///    boundary-safe textual mention must exist.
...
pub async fn match_event(
    ...
    for (index, rule) in rules.iter().enumerate() {
        ...
        // 3. Mention check. Structured `p` tags take precedence; textual
        //    compatibility is deliberately narrower and fails closed.
        if rule.require_mention
            && !event_mentions_agent(event, agent_pubkey_hex, text_mention_name, is_dm)
        {
            continue;
        }
...
/// Return whether an event contains an accepted structured or textual agent mention.
pub(crate) fn event_mentions_agent(
    ...
    let mut has_p_tag = false;
    for tag in event.tags.iter() {
        let fields = tag.as_slice();
        if fields.first().map(|value| value.as_str()) != Some("p") {
            continue;
        }
        has_p_tag = true;
        if fields.get(1).map(|value| value.as_str()) == Some(agent_pubkey_hex) {
            return true;
        }
    }

    if has_p_tag || is_dm || event.kind.as_u16() as u32 != KIND_STREAM_MESSAGE {
        return false;
    }

    let Some(display_name) = text_mention_name else {
        return false;
    };
    matches_text_mention(&event.content, display_name)
}

fn matches_text_mention(content: &str, display_name: &str) -> bool {
    if display_name.is_empty() {
        return false;
    }
    let folded_content: String = content.chars().flat_map(char::to_lowercase).collect();
    let folded_name: String = display_name.chars().flat_map(char::to_lowercase).collect();
    let needle = format!("@{folded_name}");
    let spans: Vec<(usize, usize)> = folded_content
        .match_indices(&needle)
        .filter_map(|(start, matched)| {
            let end = start + matched.len();
            let before = folded_content[..start].chars().next_back();
            let after = folded_content[end..].chars().next();
            let boundary_ok = before.is_none_or(|ch| !is_identifier_char(ch))
                && after.is_none_or(|ch| !is_identifier_char(ch));
            boundary_ok.then_some((start, end))
        })
        .collect();
    ...
    let mut remainder = String::with_capacity(folded_content.len());
    let mut cursor = 0;
    for (start, end) in spans {
        remainder.push_str(&folded_content[cursor..start]);
        cursor = end;
    }
    remainder.push_str(&folded_content[cursor..]);

    !matches!(remainder.trim(), "!shutdown" | "!cancel" | "!rotate")
}
...
mod tests {
    ...
    async fn test_match_event_first_match_wins() {
        ...
        let matched = match_event(&event, channel_id, &rules, "", None, false)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_match_event_accepts_enabled_text_mention_without_p_tags() {
        let event = make_event(9, "Could @mAvErIcK Prime review this?");
        let channel_id = any_channel();
        let rules = vec![make_rule(
            "mention-only",
            ChannelScope::All("all".into()),
        ...
        .await
        .expect("boundary-safe textual mention should match");

        assert_eq!(matched.prompt_tag, "mentioned");
    }

    #[test]
    fn test_text_mentions_fail_closed_outside_compatibility_scope() {
        let agent_pubkey = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
        ...
        for content in [
            ...
            let event = make_event(9, content);
            assert!(
                !event_mentions_agent(&event, agent_pubkey, Some("Maverick"), false),
                "must reject {content:?}"
            );
        }

        let event = make_event(9, "hello, @MAVERICK!");
        assert!(event_mentions_agent(
            &event,
            agent_pubkey,
            Some("Maverick"),
            false
        ...
        assert!(!event_mentions_agent(
            &make_event(1, "@Maverick"),
            agent_pubkey,
            Some("Maverick"),
            false
        ));
        assert!(!event_mentions_agent(
            &make_event_with_p_tag(9, "@Maverick", foreign_pubkey),
            agent_pubkey,
            Some("Maverick"),
            false
        ));
        assert!(!event_mentions_agent(&event, agent_pubkey, None, false));
    }
    ...
    async fn test_match_event_no_match() {
        ...
        let result = match_event(&event, channel_id, &rules, "", None, false).await;
        assert!(result.is_none());
    }
