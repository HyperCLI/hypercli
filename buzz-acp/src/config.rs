...
pub struct CliArgs {
    ...
    /// Display name used by opt-in textual @mention compatibility.
    #[arg(long, env = "BUZZ_ACP_DISPLAY_NAME")]
    pub display_name: Option<String>,

    /// Accept boundary-safe textual @mentions when no structured `p` tag exists.
    #[arg(long, env = "BUZZ_ACP_TEXT_MENTIONS", default_value_t = false)]
    pub text_mentions: bool,

    /// Remind external agents that end a turn without a Buzz publish attempt.
...
pub struct ChannelFilter {
    ...
    /// Whether the relay subscription must be broad enough to receive textual mentions.
    pub accept_text_mentions: bool,
}
...
pub struct Config {
    ...
    pub no_mention_filter: bool,
    /// Validated display name for textual @mention compatibility.
    pub text_mention_name: Option<String>,
    /// Apply Buzz's bounded reply guard to external ACP turns.
    pub require_reply: bool,
...
const TEXT_MENTION_NAME_MAX_CHARS: usize = 80;
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        let text_mention_name = if args.text_mentions {
            let display_name = args
                ...
                .ok_or_else(|| {
                    ConfigError::ConfigFile(
                        "BUZZ_ACP_TEXT_MENTIONS requires a non-empty BUZZ_ACP_DISPLAY_NAME".into(),
                    )
                })?;
            if display_name.contains('@')
                || display_name.chars().any(char::is_control)
                || display_name.chars().count() > TEXT_MENTION_NAME_MAX_CHARS
            {
                return Err(ConfigError::ConfigFile(format!(
                    "BUZZ_ACP_DISPLAY_NAME must be at most {TEXT_MENTION_NAME_MAX_CHARS} \
                         characters and must not contain '@' or control characters"
                )));
            }
                ...
                result.insert(
                    *ch,
                    ChannelFilter {
                        ...
                        require_mention,
                        accept_text_mentions: config.text_mention_name.is_some(),
                    },
                );
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_text_mentions_broaden_relay_filter_without_weakening_local_rule() {
        let mut config = test_config(SubscribeMode::Mentions);
        config.text_mention_name = Some("Maverick Prime".into());
        let channel = Uuid::new_v4();

        let result = resolve_channel_filters(&config, &[channel], &[]);
        let filter = result.get(&channel).expect("channel should be present");

        assert!(
            filter.require_mention,
            "local mention rule must remain enabled"
        );
        assert!(
            filter.accept_text_mentions,
            "relay must admit no-p-tag messages for local text matching"
        );
    }
...
"#,
    ...
    #[test]
    fn text_mentions_full_path_requires_valid_display_name() {
        for display_name in [None, Some("   "), Some("Mav@rick")] {
            let mut argv = vec![
                "buzz-acp",
                "--private-key",
                TEST_PRIVATE_KEY,
            ...
            if let Some(name) = display_name {
                argv.extend(["--display-name", name]);
            }
            let args = CliArgs::try_parse_from(argv).expect("clap should parse args");
            let error = Config::from_args(args).expect_err("invalid text mention config must fail");
            assert!(error.to_string().contains("DISPLAY_NAME"));
        }
    }

    #[test]
    fn text_mentions_full_path_trims_and_enables_display_name() {
        ...
        let config = Config::from_args(args).expect("valid text mention config should parse");
        assert_eq!(config.text_mention_name.as_deref(), Some("Maverick Prime"));
    }

    #[test]
    fn text_mentions_full_path_rejects_overlong_display_name() {
        let display_name = "m".repeat(TEXT_MENTION_NAME_MAX_CHARS + 1);
        let args = CliArgs::try_parse_from([
            "buzz-acp",
            "--private-key",
            TEST_PRIVATE_KEY,
        ...
        let error = Config::from_args(args).expect_err("overlong display name must fail");
