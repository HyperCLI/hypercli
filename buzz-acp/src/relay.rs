...
/// - `#p` is included only for structured-only mention subscriptions.
...
async fn send_subscribe(
    ...
    // #p — omit when textual mentions are enabled so no-tag events reach the local matcher.
    if filter.require_mention && !filter.accept_text_mentions {
...
mod tests {
    ...
    fn test_channel_filter() -> ChannelFilter {
        ChannelFilter {
            ...
            accept_text_mentions: false,
        }
    }

    #[tokio::test]
    async fn text_mention_subscription_omits_p_filter_only_when_enabled() {
        let (mut client, mut server) = test_ws_pair().await;
        let state = BgState::new();
        let channel_id = Uuid::new_v4();
        let structured_only = ChannelFilter {
            kinds: Some(vec![9]),
        ...
        let structured_req = next_test_frame(&mut server).await;
        assert_eq!(structured_req[2]["#p"], json!(["agent-pubkey"]));

        let text_compatible = ChannelFilter {
            accept_text_mentions: true,
            ..structured_only
        };
        assert!(
            send_subscribe(
        ...
        let text_req = next_test_frame(&mut server).await;
        assert!(text_req[2].get("#p").is_none());
    }
