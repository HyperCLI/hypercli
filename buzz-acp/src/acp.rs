...
pub struct AcpClient {
    ...
    /// Whether the current logical turn attempted a Buzz publish command.
    /// Persists across bounded reply-guard prompts and resets for each user turn.
    reply_publish_attempted: bool,
...
impl AcpClient {
    ...
    pub async fn session_prompt_blocks_with_idle_timeout(
        ...
        self.session_prompt_blocks_with_reply_guard(
            session_id,
            prompt_blocks,
            idle_timeout,
            max_duration,
    ...
    /// Send a logical turn and optionally apply Buzz's bounded reply guard.
    ///
    /// The guard re-prompts the same ACP session at most twice when the agent
    /// returns a genuine `end_turn` without attempting `buzz messages send` or
    /// `buzz reactions add`. All attempts share one hard deadline. Streamed
    /// assistant text is observational and is never published automatically.
    pub async fn session_prompt_blocks_with_reply_guard(
        &mut self,
        session_id: &str,
        prompt_blocks: &[&str],
        idle_timeout: std::time::Duration,
        ...
        self.reply_publish_attempted = false;
        let mut reply_nags = 0;
        let guard_blocks = [REPLY_GUARD_NAG];
        ...
        loop {
            let current_blocks = if reply_nags == 0 {
                prompt_blocks
            } else {
                &guard_blocks
            };
            let params = build_prompt_params(session_id, current_blocks);
            ...
            let result = self
                .read_until_response_with_idle_timeout_and_steer(
                    session_id,
                    id,
                    idle_timeout,
                    hard_deadline,
            ...
            match &stop_reason {
                Ok(StopReason::EndTurn)
                    if require_reply
                        && !self.reply_publish_attempted
                        && reply_nags < MAX_REPLY_NAGS =>
                {
                    reply_nags += 1;
                    tracing::info!(
                        target: "acp::reply_guard",
                        "agent ended without a Buzz publish attempt; sending reminder {reply_nags}/{MAX_REPLY_NAGS}"
                    );
                    continue;
                }
                Err(AcpError::IdleTimeout(_) | AcpError::HardTimeout { .. }) => {
                    // Leave prompt/deadline state for cancel_with_cleanup.
                }
                _ => {
                    self.last_prompt_id = None;
                    self.current_hard_deadline = None;
    ...
    async fn read_until_response_with_idle_timeout(
        ...
        let mut steer_rx = self.steer_rx.take();
        self.read_until_response_with_idle_timeout_and_steer(
            session_id,
            expected_id,
            idle_timeout,
            hard_deadline,
    ...
    async fn read_until_response_with_idle_timeout_and_steer(
        &mut self,
        session_id: &str,
        expected_id: u64,
        idle_timeout: std::time::Duration,
        hard_deadline: tokio::time::Instant,
        max_duration: std::time::Duration,
        steer_rx: &mut Option<tokio::sync::mpsc::Receiver<crate::pool::SteerRequest>>,
    ) -> Result<serde_json::Value, AcpError> {
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        if matches!(update_type, "tool_call" | "tool_call_update")
            && update_is_buzz_publish_attempt(update)
        {
            self.reply_publish_attempted = true;
            tracing::debug!(
                target: "acp::reply_guard",
                "observed Buzz publish-shaped ACP tool call"
            );
        }
...
/// Return whether an ACP tool update attempts to publish a Buzz reply.
///
/// External adapters expose shell input in one of two source-grounded shapes:
/// direct `rawInput.command`, or Codex MCP's nested
/// `rawInput.arguments.command`. Kimi may add `rawInput` only on a later
/// `tool_call_update`, so both initial and update frames are inspected.
fn update_is_buzz_publish_attempt(update: &serde_json::Value) -> bool {
    let command = update
        .pointer("/rawInput/command")
        .or_else(|| update.pointer("/rawInput/arguments/command"))
...
mod tests {
    ...
    async fn spawn_named_and_read_child_env(
        ...
        // Keep the runtime-bearing file name while executing the system shell.
        // A freshly written executable can intermittently fail with ETXTBSY on
        // Linux overlay filesystems even after its writer has been dropped.
        std::os::unix::fs::symlink("/bin/sh", &path).expect("create env probe shell symlink");
    ...
    async fn keepalive_resets_idle_past_deadline() {
        ...
        // Leave enough scheduling margin for this subprocess-based test under
        // nextest's parallel CI load while still proving repeated idle resets.
        let mut client = spawn_script(
