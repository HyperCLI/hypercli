...
def test_start_rejects_launch_config_argument():
    deployments = Deployments(
...
def test_build_agent_config_omits_empty_remove_headers_from_generic_routes():
    ...
    assert launch_config["routes"] == {
        "web": {"port": 3000, "auth": True, "prefix": ""},
        "api": {"port": 8000, "remove_headers": ["X-Test"]},
    }


def test_build_openclaw_trusted_proxies_env_is_comma_separated_replacement():
    assert build_openclaw_trusted_proxies_env(None) == {}
    assert build_openclaw_trusted_proxies_env([]) == {}
    assert build_openclaw_trusted_proxies_env([" 10.0.0.0/8 ", "", "127.0.0.1"]) == {
...
def test_stored_launch_config_rehydrates_redacted_projection_round_trip(agents_client):
    """stored_launch_config() repairs owner projections for explicit update."""
    ...
    launch_config = agents_client.stored_launch_config(_STORED_AGENT_ID)

    assert launch_config["secrets"] == {"API_TOKEN": "tok"}
    assert launch_config["registry_auth"] == {}
    assert launch_config["env"] == {"MODE": "prod"}
...
def test_start_honours_explicitly_empty_redactable_keys(agents_client):
    ...
    assert any(path.endswith("/secrets") for path in calls)


def test_start_refuses_to_invent_registry_auth_for_private_registry(agents_client):
    stored = build_agent_config(
        image="registry.example.com/hypercli/private:sha",
...
def test_agents_start_preserves_generic_launch_fields(agents_client):
    with (
        ...
        agent = agents_client.start("agent-456")
...
def test_agents_update_preserves_sync_policy_presence(agents_client, kwargs, expected):
    patched: dict = {}
    posted: dict = {}
    ...
    agents_client.update("11111111-1111-4111-8111-111111111111", launch_config=launch_config)
...
def test_build_agent_config_rejects_sync_none_shapes(agents_client):
    base = build_agent_config()

    with pytest.raises(ValueError, match="sync_include must contain"):
        _copy_complete_launch_config({**base, "sync_include": []})
    with pytest.raises(ValueError, match="exclude the entire sync root"):
        _copy_complete_launch_config({**base, "sync_exclude": ["**"]})
    with pytest.raises(ValueError, match="exclude the entire sync root"):
        _copy_complete_launch_config({**base, "sync_exclude": ["*"]})
...
def test_agents_start_retains_backend_hydrated_launch_config(agents_client):
    with (
        ...
        agent = agents_client.start("11111111-1111-4111-8111-111111111111")
