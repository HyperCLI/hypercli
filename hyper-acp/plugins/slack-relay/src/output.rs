...
use crate::reply::{
    build_assistant_thread_status_proxy_request, plan_slack_reply_deliveries,
    SlackRelayApiProxyRequest, SlackRelayHttpSender, SlackReplyDelivery, SlackReplyDeliveryError,
    SlackReplyPayload, SlackReplyToMode, SLACK_TEXT_LIMIT,
};
...
impl SlackAcpOutputConfig {
    ...
    /// # Errors
    ///
    /// Returns an error when `HYPER_AGENTS_API_KEY` is absent or URL derivation fails.
    pub fn from_env(relay_url: Option<&str>) -> Result<Option<Self>, SlackAcpOutputError> {
        ...
        let Some(relay_api_base_url) = relay_api_base_url else {
            return Ok(None);
        };
        let hyper_agents_api_key = env_var(HYPER_AGENTS_API_KEY_ENV)
            .ok_or(SlackAcpOutputError::MissingEnv(HYPER_AGENTS_API_KEY_ENV))?;
...
pub enum SlackAcpOutputDelivery {
    ...
    Status(SlackRelayApiProxyRequest),
}
...
pub async fn run_slack_acp_output_to_replies(
    ...
    while let Some(frame) = frames.recv().await {
        let deliveries = state.process_frame(&config, &frame)?;
        for delivery in deliveries {
            match delivery {
                SlackAcpOutputDelivery::Reply(reply) => {
                    sender.send(&reply.request).await?;
                }
                SlackAcpOutputDelivery::Status(request) => {
                    let _result = sender.send(&request).await;
                }
            }
        }
    }
...
impl SlackOutputTurn {
    fn plan_final_replies(&self, config: &SlackAcpOutputConfig) -> Vec<SlackAcpOutputDelivery> {
        ...
        plan_slack_reply_deliveries(
            &config.relay_api_base_url,
            &config.hyper_agents_api_key,
            &self.channel,
    ...
    fn plan_status(
        ...
        vec![SlackAcpOutputDelivery::Status(
            build_assistant_thread_status_proxy_request(
                &config.relay_api_base_url,
                &config.hyper_agents_api_key,
                &self.channel,
                thread_ts,
...
fn read_hypercli_slack_meta(params: &Value) -> Option<&Value> {
    params
        .get("_meta")
        .and_then(|meta| meta.get("hypercli.slack"))
        .or_else(|| {
...
fn status_text_for_update(update: &Value) -> &'static str {
    match session_update_kind(update) {
...
fn status_text_for_state_update(update: &Value) -> &'static str {
...
mod tests {
    ...
    fn config() -> SlackAcpOutputConfig {
        SlackAcpOutputConfig {
            relay_api_base_url: "https://relay.example".to_owned(),
            hyper_agents_api_key: "key".to_owned(),
            text_limit: 4,
            reply_to_mode: SlackReplyToMode::All,
        }
    }
    ...
    fn correlates_prompt_updates_and_completion_into_chunked_reply() {
        ...
        assert_eq!(first.request.body["text"], "abcd");
        assert_eq!(first.request.body["thread_ts"], "100.1");
    }
    ...
    fn handles_canonical_update_kinds_reasoning_and_structured_payloads() {
        ...
        let reply = deliveries
            ...
            .find_map(|delivery| match delivery {
                SlackAcpOutputDelivery::Reply(reply)
                    if reply.request.body["metadata"]["event_payload"]["openclaw_delivery_id"]
                        == "dq1" =>
                {
                    Some(reply)
                }
        ...
        assert_eq!(reply.request.body["thread_ts"], "200.2");
        assert_eq!(
            reply.request.body["metadata"]["event_payload"]["openclaw_delivery_id"],
            "dq1"
        );
        assert_eq!(reply.request.body["text"], "Photo");
