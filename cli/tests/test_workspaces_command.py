...
def test_workspaces_explicit_product_key_beats_configured_agent_key(monkeypatch, tmp_path):
    ...
    config_path = tmp_path / ".hypercli" / "config"
    config_path.parent.mkdir()
    config_path.write_text("HYPER_AGENTS_API_KEY=hyper_api_configured_agent\n")
    monkeypatch.setattr(config_mod, "CONFIG_FILE", config_path)
...
def test_workspaces_without_runtime_key_retains_product_config(monkeypatch, tmp_path):
    import hypercli.config as config_mod
    import hypercli_cli.workspaces as workspaces_mod

    config_path = tmp_path / ".hypercli" / "config"
    config_path.parent.mkdir()
    config_path.write_text("HYPER_API_KEY=hyper_api_product_config\n")
    monkeypatch.setattr(config_mod, "CONFIG_FILE", config_path)
    monkeypatch.delenv("HYPER_AGENTS_API_KEY", raising=False)
    monkeypatch.delenv("HYPER_API_KEY", raising=False)
    monkeypatch.delenv("HYPERCLI_API_KEY", raising=False)
