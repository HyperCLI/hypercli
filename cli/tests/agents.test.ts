...
describe('hyper agents web-search', () => {
  it('prints the title/url table in table mode', async () => {
    const d = createMockDeploymentsApi([agentFixture()]);
    const { ctx } = makeCtx(fakeClient({ deployments: d }), 'table');

    await agents.run(ctx, ['web-search', 'HyperCLI Brave agent container smoke']);
  ...
  it('joins multi-word positionals into one query', async () => {
    ...
    await agents.run(ctx, ['web-search', 'hypercli', 'brave', 'smoke']);

    expect(d.webSearch).toHaveBeenCalledWith('hypercli brave smoke', { count: 5 });
  });

  it('passes --count through to d.webSearch', async () => {
    const d = createMockDeploymentsApi([agentFixture()]);
    const { ctx } = makeCtx(fakeClient({ deployments: d }), 'table');

    await agents.run(ctx, ['web-search', 'hypercli', '--count', '1']);
  ...
  it('--json emits the raw Brave payload, parseable on stdout', async () => {
    ...
    await agents.run(ctx, ['web-search', 'HyperCLI Brave agent container smoke', '--count', '1', '--json']);
  ...
  it('table mode with no results prints an empty state', async () => {
    ...
    await agents.run(ctx, ['web-search', 'hypercli']);
  ...
  it('missing query and out-of-range --count are UsageError (exit 2)', async () => {
    ...
    const missing = await runErr(ctx, ['web-search', '--count', '1']);
    expect(missing).toBeInstanceOf(UsageError);
    expect(exitCodeFor(missing)).toBe(2);

    for (const bad of ['0', '21', 'abc']) {
      const err = await runErr(ctx, ['web-search', 'hypercli', '--count', bad]);
      expect(err).toBeInstanceOf(UsageError);
      expect(exitCodeFor(err)).toBe(2);
    }
  ...
  it('API failure is a CliError naming the failing call', async () => {
    ...
    const err = await runErr(ctx, ['web-search', 'hypercli']);
