...
app = typer.Typer(help="Manage shared knowledge")
console = Console()
...
def _parse_workspace_file_ref(value: str) -> tuple[str, str]:
    ...
    if "/" not in normalized:
        raise typer.BadParameter("Expected shared knowledge slug/path")
...
def create_workspace(
    name: str = typer.Argument(help="Shared knowledge display name"),
    ...
    if output == "json":
        ...
        return
    console.print(f"[green]Created shared knowledge[/green] {workspace.slug} ({workspace.id})")
...
def list_workspaces(
    ...
    """List accessible shared knowledge."""
    ...
    if output == "json":
        ...
        return
    table = Table(title="Shared knowledge")
...
def search_workspaces(
    query: str = typer.Argument(help="Search shared knowledge names, file paths, filenames, and file metadata"),
...
def search_files(
    workspace: str = typer.Argument(help="Shared knowledge slug or ID"),
    ...
    """Search files in shared knowledge."""
    ...
    if output == "json":
        ...
        return
    table = Table(title=f"Shared knowledge file search: {workspace} / {query}")
...
def update_workspace(
    workspace: str = typer.Argument(help="Shared knowledge slug or ID"),
    name: str | None = typer.Option(None, "--name", help="New shared knowledge display name"),
    ...
    if output == "json":
        ...
        return
    console.print(f"[green]Updated shared knowledge[/green] {updated.slug} ({updated.id})")
...
def delete_workspace(
    ...
    """Soft-delete shared knowledge."""
    _get_workspaces().delete_workspace(workspace, user_id=user_id)
    console.print(f"[red]Deleted shared knowledge[/red] {workspace}")
...
def grant_workspace(
    ...
    """Grant shared knowledge access to an agent or user."""
...
def revoke_workspace_grant(
    ...
    """Revoke a shared knowledge grant."""
...
def upload(
    ...
    path: str | None = typer.Option(None, "--path", help="Shared knowledge-relative destination path"),
    user_id: str | None = typer.Option(None, "--user-id", help="Explicit acting user subject for local/dev testing"),
    output: str = typer.Option("table", "--output", "-o", help="Output format: table|json"),
):
    """Upload a local source file into shared knowledge."""
...
def manifest(
    ...
    """Print a shared knowledge Markdown manifest."""
...
def wait_until_processed(
    workspace: str = typer.Argument(help="Shared knowledge slug or ID"),
    file_ref: str = typer.Argument(help="Shared knowledge-relative source path or file ID"),
    ...
    """Poll a shared knowledge file until its Markdown processing is finished."""
...
def sync(
    ...
    """Sync shared knowledge Markdown files to a local directory."""
    user_id, agent_id = _resolve_auth_subject(user_id, agent_id)
    if all_workspaces:
        if workspace:
            raise typer.BadParameter("Pass either a shared knowledge argument or --all, not both")
        ...
        console.print(f"[green]Synced[/green] {total} Markdown file(s) from {len(synced)} shared knowledge source(s) to {output_dir}")
        for slug, paths in synced.items():
            console.print(f"  {slug}: {len(paths)}")
        return
    if not workspace:
        raise typer.BadParameter("Pass a shared knowledge argument or --all")
...
def download(
    workspace: str | None = typer.Argument(None, help="Shared knowledge slug or ID, or omit to read slug/path from stdin"),
    file_ref: str | None = typer.Argument(None, help="Shared knowledge-relative source path or file ID"),
    ...
    if workspace is None or file_ref is None:
        raise typer.BadParameter("Pass a shared knowledge slug and file_ref, or pipe slug/path on stdin")
    if raw and md:
...
@app.command("enrich")
def enrich(
    address: str | None = typer.Argument(None, help="Shared knowledge file address: slug/path, or omit to read stdin"),
    ...
    """Build a shared knowledge enrichment payload from generated Markdown files."""
    resolved_address = address or sys.stdin.read().strip()
    if not resolved_address:
        raise typer.BadParameter("Pass a shared knowledge slug/path or pipe it on stdin")
...
def regenerate(
    ...
    """Queue Markdown regeneration for a shared knowledge file."""
...
def delete_file(
    ...
    """Soft-delete a shared knowledge file."""
