 ...
 * and you get `ws://localhost:1420/ws`, which is the Vite server, not the API.
 */
import type { AcpCredentials } from "../api";

export interface Endpoints {
  /** Base for REST. Same-origin in dev, the real gateway when packaged. */
  httpBase: string;
...
/** Path prefixes `vite.config.ts` proxies upstream. Keep the two in step. */
...
function devHttpBase(apiBase: string): string {
  ...
  // strips `/agents` and appends `/routines` — keep working unchanged.
