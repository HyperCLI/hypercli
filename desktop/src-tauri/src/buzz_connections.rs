...
impl<S: OwnerSecretStore> BuzzConnectionRepository<S> {
    ...
    // Used by the delete flow (not yet ported from the original desktop app).
    #[allow(dead_code)]
...
pub fn canonical_relay_url(value: &str) -> Result<String, BuzzConnectionError> {
    ...
    let relay = RelayUrl::parse(&with_scheme).map_err(|_| {
        BuzzConnectionError::InvalidInput(
            "community relay must be a valid hostname or ws:// or wss:// URL".into(),
        )
    })?;
...
mod tests {
    ...
    #[test]
    fn relay_url_accepts_bare_hostname_and_defaults_to_wss() {
        assert_eq!(
            canonical_relay_url("dev.buzz.hypercli.com").unwrap(),
            "wss://dev.buzz.hypercli.com"
        );
        assert_eq!(
            canonical_relay_url("wss://dev.buzz.hypercli.com").unwrap(),
            "wss://dev.buzz.hypercli.com"
        );
        assert_eq!(
            canonical_relay_url(" ws://localhost:3000 ").unwrap(),
            "ws://localhost:3000"
        );
        assert!(canonical_relay_url("https://dev.buzz.hypercli.com").is_err());
        assert!(canonical_relay_url("").is_err());
    }
