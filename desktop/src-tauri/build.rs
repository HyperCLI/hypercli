fn main() {
    // Env-gated updater: the updater + process plugins are registered in
    // lib.rs only when this cfg is present, and the cfg is emitted only when
    // both env vars are non-empty at build time. Local and dev builds without
    // them ship with the updater compiled out.
